...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: AFRO.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: AFROd.pid)") + "\n";
    ...
    strUsage += "  -maxconnections=<n>    " + _("Maintain at most <n> connections to peers (default: 500)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeAFROamount=<n> " + _("Keep N AFRO anonymized (default: 0)") + "\n";
    ...
    strUsage += "  -stakethreshold=<n> " + _("This will set the output size of your stakes to never be below this number (default: 17777)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. AFRO is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. AFRO is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("AFRO version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "AFRO server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of AFRO") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart AFRO to complete") << "\n";
    ...
    nAnonymizeAFROAmount = GetArg("-anonymizeAFROamount", 0);
    if(nAnonymizeAFROAmount > 999999) nAnonymizeAFROAmount = 999999;
    if(nAnonymizeAFROAmount < 2) nAnonymizeAFROAmount = 2;
    ...
    LogPrintf("Anonymize AFRO Amount %d\n", nAnonymizeAFROAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1AFRO+1000 == (.1AFRO+100)*10
       10AFRO+10000 == (1AFRO+1000)*10
    */
