...
//
// Bootup the masternode, look for a 500 AFRO input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in AFRO.conf */
...
{
    CAFROAddress address(collateralAddress);
