...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeAFROAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeAFROAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        //strAnonymizeAFROAmount = strAnonymizeAFROAmount.remove(strAnonymizeAFROAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAFROAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeAFROAmount*COIN) nMaxToAnonymize = nAnonymizeAFROAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeAFROAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeAFROAmount));
        //strAnonymizeAFROAmount = strAnonymizeAFROAmount.remove(strAnonymizeAFROAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAFROAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
