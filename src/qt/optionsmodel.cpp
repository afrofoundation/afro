...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeAFROAmount"))
        settings.setValue("nAnonymizeAFROAmount", 1000);
    nAnonymizeAFROAmount = settings.value("nAnonymizeAFROAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeAFROAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeAFROAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeAFROAmount:
            return QVariant(nAnonymizeAFROAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeAFROAmount:
            nAnonymizeAFROAmount = value.toInt();
            settings.setValue("nAnonymizeAFROAmount", nAnonymizeAFROAmount);
            emit AnonymizeAFROAmountChanged(nAnonymizeAFROAmount);
            break;
        default:
            break;
        }
