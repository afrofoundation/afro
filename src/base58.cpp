...
namespace {
    class CAFROAddressVisitor : public boost::static_visitor<bool> {
    private:
        CAFROAddress *addr;
    public:
        CAFROAddressVisitor(CAFROAddress *addrIn) : addr(addrIn) { }
...
bool CAFROAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CAFROAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CAFROAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CAFROAddressVisitor(this), dest);
}

bool CAFROAddress::IsValid() const {
...
CTxDestination CAFROAddress::Get() const {
    if (!IsValid())
...
bool CAFROAddress::GetKeyID(CKeyID &keyID) const {
...
bool CAFROAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CAFROSecret::SetKey(const CKey& vchSecret) {
...
CKey CAFROSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CAFROSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CAFROSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CAFROSecret::SetString(const std::string& strSecret) {
